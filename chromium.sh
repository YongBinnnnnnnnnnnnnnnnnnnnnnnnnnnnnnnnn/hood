...
sed -e "s|font-family\b|$(cat /dev/random | tr -dc "@#%^*_+=0-9A-Za-z"|head -c 11)|g" -e "s|canvas\b|$(cat /dev/random | tr -dc "@#%^*_+=0-9A-Za-z"|head -c 6)|g" $executable_path/$executable > /tmp/ychrome/ychrome
