...
sed -e "s|font-family|$(cat /dev/random | tr -dc "@#%^*_+=0-9A-Za-z"|head -c 11)|g" -e "s|HTMLCanvasElement|$(cat /dev/random | tr -dc "@#%^*_+=0-9A-Za-z"|head -c 17)|g" $executable_path/$executable > /tmp/ychrome/ychrome
