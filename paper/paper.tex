...
By design, only HTTP and HTTPS protocols are allowed to pass-through the firewall. A proxy is used to filter, protect HTTP plaintext connections. The proxy check the host name with a known list of OCSP servers. Only the connections to OCSP servers are allowed to be plaintext. The connections to other hosts will be converted to HTTPS connections. If the remote host accept only HTTP connections, the communication attempt will be failed.
