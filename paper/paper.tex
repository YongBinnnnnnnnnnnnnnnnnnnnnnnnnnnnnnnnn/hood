...
\end{figure}
\paragraph{}
I also collected package names of Fedora 39 security updates between February 14th, 2020 and January 10th, 2024, and then counted the number of times a prefix was mentioned. See \cref{fig:fedora-sec-update}. It the data is not complete as I only collected the titles of the updates and some titles include multiple packages like "a, b, c & X more". In this case, the "X more" is not counted, but the result can still spit out some truth because the more frequent a component causes a security update, the less chance it is being published with a lot of other packages. The result also proves the high risk of browsers: both "firefox and "chromium" are in the top 10. The result also seemingly indicates the high risk of Python as "python" is 1st place, which renders the use of Python as a mistake, but the "python" prefix is shared by both the packages of multiple Python versions and the packages of different Python libraries. To verify the true risk of using Python, I additionally counted the times of the prefix "python3.11" being mentioned and the result is only twice.
...
\section{Conclusions}
As conclusion, the decisions on disabling things are based on the analyzation of differnt security events and supported by numbers and related threats.
